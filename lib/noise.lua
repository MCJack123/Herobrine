...
local function random(min, max)
	local x = bit32.band(r[(n + 313) % 344] + r[(n + 341) % 344], 0xFFFFFFFF)
	r[n % 344] = x
	n = (n + 1) % 344
	return bit32.rshift(x, 1) / 2147483647 * max
end

local function randomseed(s)
	next = s
	r[0] = s
	for i = 1, 30 do
	...
	for i = 34, 343 do
		r[i] = bit32.band(r[i - 31] + r[i - 3], 0xFFFFFFFF)
	end
	n = 0
end

local function randomValue()
	return random(0, 1023) / 1023
end

local function getRawLayer(layerWidth, x, y, seed)
	randomseed(seed + x * 1000 + y * 1000000)
	local prelayer = {}
